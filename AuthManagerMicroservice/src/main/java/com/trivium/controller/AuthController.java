...
public class AuthController {
	...
	public ResponseEntity<UserDTO> register(@RequestBody UserDTO userDTO) {
		User user = userService.registerUser(userDTO.getUsername(), userDTO.getPassword(), userDTO.getEmail(),
				userDTO.getRole(), userDTO.getHouseName(), userDTO.getStreet(), userDTO.getDistrict(),
				userDTO.getState(), userDTO.getZipcode());
	...
	public ResponseEntity<UserDTO> updateProfile(@RequestBody UserDTO userDTO) {
		...
		if (addr != null) {
			...
			if (userDTO.getHouseName() != null)
				user.getAddress().setHouseName(userDTO.getHouseName());
			if (userDTO.getStreet() != null)
				user.getAddress().setStreet(userDTO.getStreet());
			if (userDTO.getDistrict() != null)
				user.getAddress().setDistrict(userDTO.getDistrict());
	...
	public ResponseEntity<?> updateUserByAdmin(@PathVariable String username, @RequestBody UserDTO userDTO,
		...
		User admin = userService.findByUsername(adminUsername).orElse(null);

		if (admin == null || !jwtUtil.validateToken(token, userService.loadUserByUsername(adminUsername))) {
		...
		Address addr = user.getAddress();
		if (userDTO.getHouseName() != null)
			addr.setHouseName(userDTO.getHouseName());
		if (userDTO.getStreet() != null)
			addr.setStreet(userDTO.getStreet());
		if (userDTO.getDistrict() != null)
			addr.setDistrict(userDTO.getDistrict());
		if (userDTO.getState() != null)
			addr.setState(userDTO.getState());
		if (userDTO.getZipcode() != null)
			addr.setZipcode(userDTO.getZipcode());
